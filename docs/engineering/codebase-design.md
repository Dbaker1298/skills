...
The skill does not answer it, and that is the gap worth knowing about before you start. It defines what a deep module *is*; it says nothing about how to stop a stray import from reaching past the interface, and without a guardrail both people and models erode an interface over time. Three mechanisms work, in rough order of what they cost you: wrap the module in a class or a closure and accept that the class grows large; make it a package in a monorepo and accept the monorepo tooling; or have a linter such as [dependency-cruiser](https://github.com/sverweij/dependency-cruiser) forbid the imports that bypass the interface. `setup-ts-deep-modules`, in the `in-progress/` bucket, takes the third route: it lays down a `src/packages/<name>/` convention where a package's root files are public and every subfolder is private, and it ships a `dependency-cruiser.config.cjs` that enforces exactly that. It is a beta skill with no docs page, so read it before you run it.
