...
Step 1 depends on `docs/agents/issue-tracker.md`, which [setup-david-baker-skills](./setup-david-baker-skills.md) writes. Without it the axis still works if you hand it a path. With no spec at all, the Spec sub-agent is skipped and the report says "no spec available" rather than inventing requirements.
...
A name collision with no clean fix. Claude Code ships its own `/code-review`, which does something different: it hunts bugs in the diff, where this one checks spec compliance and repo standards. Installing this library means one of them wins, and which one wins depends on how you installed. Via the plugin marketplace, everything is aliased under a `david-baker-skills:` prefix and the built-in becomes hard to reach at the unqualified name; via a plain skills install, the local file wins and this skill shadows the built-in. One clean answer is to remove Claude Code's built-in skills entirely: a large context saving, and the collision stops mattering. The shadowing itself is arguably a Claude Code harness bug (a skill author should be free to name a skill anything), so the other answer is to rename the local copy. Editing the frontmatter or renaming the directory gets undone the next time you update the skill, so the durable version is to fork it to a new name and drop `code-review` from the managed set, keeping a note of the commit you forked from so you can re-sync by hand.

**Its sub-agents keep invoking `/code-review` again and spawn more agents.**

Read the sub-agent briefs in `SKILL.md` and you can see why. Neither the Standards brief nor the Spec brief forbids delegation, so a sub-agent that goes looking can rediscover this same skill and fan out again, and each of those can do it once more. Nothing bounds the depth. One line appended to both briefs closes it: "Do not invoke `/code-review` or spawn additional agents: perform this review directly." Add it to your own copy, or handle it at the harness level so every skill inherits the guard. Neither is in the shipped skill. If you run this unattended, watch the agent count.

**Should I run it in the same session that wrote the code?**

Prefer a fresh one. The same context reviewing itself is not review, it is confirmation bias with a slash command: the agent in the authoring session holds every assumption that shaped the code, which is precisely the context an independent reviewer would lack. It is also the reason to be wary of [implement](./implement.md)'s built-in review step, which runs inside the session that just wrote the diff. Invoking `/code-review` yourself from a clean session is the honest version.
...
Not without checking. Sub-agent output is a hypothesis, not evidence. The skill aggregates the two reports verbatim or lightly cleaned rather than re-verifying each claim against the files, so a finding can cite the wrong location or overstate an impact. Read the citation on each finding before acting on it. That every finding is required to carry one (a standards rule, a smell plus its hunk, or a spec line) is what makes this checkable at all.

**Why does it find new problems every single time I run it?**

Because fixes create new surface, and because the judgement-call half of the Standards axis is not deterministic between runs. Fix what a pass finds, run it again, and it finds more, indefinitely. There is no convergence guarantee, and there is nothing in the skill that would provide one. Treat a pass as a list of leads, act on the ones with a cited rule behind them, and stop: do not run it in a loop until it comes back clean, because it will not.
