...
- **[setup-david-baker-skills](./setup-david-baker-skills/SKILL.md)**: Configure this repo for the engineering skills (issue tracker, triage labels, domain doc layout). Run once per repo.
...
- **[git-guardrails-claude-code](./git-guardrails-claude-code/SKILL.md)**: Install a Claude Code PreToolUse hook that blocks destructive git commands (`push`, `reset --hard`, `clean -f`, `branch -D`, `checkout .`) before the agent can run them.
